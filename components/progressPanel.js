...
export const ProgressPanel = ({ value, now, currentDateLimit, currentDate, foods, totalDailyNutr, dailyCal}) => {
 ...
 return(
   <View style={{flex: 1, padding: 10}}>
    <View style={{flex: 1, flexDirection: 'row', justifyContent:'space-between', }}>
     <View style={{flex: 1}}>
     <Text style={styles.numText}>{now}</Text>
     <Text style={styles.calText}>Cal intake</Text>
     </View>
     <View style={{flex: 1, alignItems: 'flex-end'}}>
     <Text style={styles.numText}>{rest}</Text>
     <Text style={styles.calText}>{progress <= 1 ? 'Cal remaining' : 'Cal over'}</Text>
     </View>
    </View>
    <ProgressBar
    ...
    <TouchableOpacity
     ...
     onPress={() => Actions.detailedPeriod({
       title:'Daily Total',
    ...
    <View style={styles.innerBlock}>
      ...
      <Text style={[styles.calText, { color: 'green' }]}>{totalProtein}</Text>
    </View>
    <View style={styles.innerBlock}>
